// Endura Roofing — LP interactions: gradient on scroll, progress, parallax, form, logo fallback, enter anim, param scroll
...
(function(){
  ...
  // Form handling (API first, mailto fallback)
...
Details: ${data.message || ''}`);
        window.location.href = `mailto:jacob@elevateestimator.com?subject=${subject}&body=${body}`;
  ...
  // Page-enter animation (staggered; motion-safe)
  (function pageEnterAnimation(){
    const reduce = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    if (reduce) return;

    const items = Array.from(document.querySelectorAll(
      'header.lp-header, main > section, footer.lp-footer, #sticky-free-estimate'
    ));
    if (!items.length) return;

    items.forEach((el) => {
      el.style.opacity = '0';
      el.style.transform = 'translateY(14px) scale(.985)';
      el.style.filter = 'blur(10px)';
      el.style.transformOrigin = '50% 20%';
      el.style.willChange = 'opacity, transform, filter';
      el.style.transition = 'transform .60s cubic-bezier(.22,.61,.36,1), opacity .60s ease, filter .60s ease';
    });
  ...
  // Scroll to #estimate if arriving with ?id=estimate
  (function scrollToEstimateIfParam(){
    const params = new URLSearchParams(location.search);
    const shouldScroll = (params.get('id') || '').toLowerCase() === 'estimate';
    ...
    setTimeout(() => {
      ...
      const firstInput = document.getElementById('name') || target.querySelector('input, textarea, select, button');
      if (firstInput) { try { firstInput.focus({ preventScroll: true }); } catch {} }
